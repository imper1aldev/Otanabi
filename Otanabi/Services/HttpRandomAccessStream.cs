...
public class HttpRandomAccessStream : IRandomAccessStreamWithContentType
{
    ...
    public static IAsyncOperation<(HttpRandomAccessStream stream, string contentType)> CreateAsync(HttpClient client, Uri uri)
    {
        return AsyncInfo.Run<(HttpRandomAccessStream, string)>(async (cancellationToken) =>
        {
            var randomStream = new HttpRandomAccessStream(client, uri);
            await randomStream.SendRequesAsync().ConfigureAwait(false);
            return (randomStream, randomStream.ContentType ?? randomStream.contentType);
        });
    }
